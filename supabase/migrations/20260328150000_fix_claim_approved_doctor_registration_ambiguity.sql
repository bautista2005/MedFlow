...
begin
  ...
  select ad.*
  into v_doctor
  from public.approved_doctors as ad
  where ad.dni = v_dni
  for update;
  ...
  update public.approved_doctors as ad
  set
    claimed_by_auth_user_id = p_auth_user_id,
    status = 'claimed'
  where ad.approved_doctor_id = v_doctor.approved_doctor_id;

  return query
  select
    v_active_doctor.active_doctor_id as active_doctor_id,
    v_doctor.approved_doctor_id as approved_doctor_id;
exception
